...
// Define types for fields and logical operators
type FieldsType = { [key: string]: string }
type OperatorsType = ('AND' | 'OR')[]
type ValuesType = (string | number)[]

class BasicModel {
	static async findBy(
		columns: string[], // Column names to select in the SQL query
		tableName: string, // Name of the table to retrieve data from
		filters: Record<string, string>, // Object with filters where keys are column names and values are filter values
		logicalOperators: string[] = [] // Array of logical operators used between conditions in the WHERE clause
		...
		if (filterEntries.length - 1 !== logicalOperators.length) {
			throw new RangeError(
				'The number of logical operators must be one less than the number of filters.'
			)
		}
	...
	static async insert(
		...
		values: any[], // Array of values to insert into the table
		columns: string[] = [] // Array of column names (optional)
	): Promise<void> {
